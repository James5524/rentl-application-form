// Run this once with `node seed-template.js` to add a ready-made "RENTL Application
// Template" form (matching James's tenant application) to the app's data file.
// Safe to run without the server running. Safe to run more than once - it always
// adds a fresh copy rather than overwriting, so re-running just gives you an extra
// template you can delete.
...
const form = {
  id: genId(),
  title: 'RENTL Application Template',
  description: '',
  fields,
  createdAt: now,
  updatedAt: now
...
console.log(`Created "RENTL Application Template" (id: ${form.id}).`);
